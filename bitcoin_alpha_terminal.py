...
st.caption("Built solo in Cedar Rapids, Iowa • Data fresh March 9, 2026 • 100% FREE forever • Prices update live on refresh")
...
st.sidebar.caption("Any amount keeps this growing from Iowa")

# ==================== LIVE DATA — ALL Public Companies (March 9, 2026) ====================
data = [
...
with tab2:
    st.subheader("Full Interactive Charts (any stock or Bitcoin)")
    ...
    if st.button("Load Full Chart with Indicators"):
        with st.spinner("Fetching real-time data + technical indicators..."):
            ...
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.7, 0.3],
                                subplot_titles=(f"{selected_ticker} Price", "RSI"))
...
with tab4:
    st.subheader("🤖 Real Grok AI Alpha Agent")
...
Analyze like Michael Saylor + Bloomberg pro. Output exactly 4 bullets:
...
4. Alert for user"""
        ...
        if xai_key:
            try:
                ...
                st.error(f"API error: {e} — falling back to demo")
                st.info("**Demo**: High probability of continued accumulation. Risk 4/10. Preferred bond opportunity: YES.")
        else:
            st.warning("Enter xAI key in sidebar for real Grok analysis (demo shown above)")
...
with tab5:
    ...
    st.caption("Every sat funds new features from Iowa")

st.divider()
st.caption("Refresh page = fresh prices • Built for you in Cedar Rapids")
